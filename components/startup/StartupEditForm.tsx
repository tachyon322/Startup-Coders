...
import { useReducer, useCallback, useState, useTransition } from "react"
...
export default function StartupEditForm({ 
  ...
  const validateForm = useCallback(() => {
    ...
    if (formState.images.length > 5) {
      errors.tags = "Максимум 5 изображений";
      isValid = false;
    }
    
    dispatch({ type: 'SET_FORM_ERRORS', payload: errors });
    return isValid;
  }, [formState.name, formState.description, formState.selectedTags, formState.images]);
  ...
  const handleSubmit = useCallback((e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        ...
        // Call success callback after a brief delay to show the toast
        setTimeout(() => {
          onSuccess();
        }, 1500);
      } catch (error) {
        ...
        dispatch({ 
          type: 'SET_ERROR', 
          payload: error instanceof Error ? error.message : "Не удалось обновить стартап. Пожалуйста, попробуйте снова." 
        });
      }
    });
  }, [validateForm, formState.name, formState.description, formState.selectedTags, formState.images, startupId, onSuccess]);
    ...
    <div className="w-full max-w-3xl mx-auto">
      ...
      >
        <div className="flex items-center">
          ...
          <div>
            ...
            <p className="text-sm">Стартап успешно обновлен.</p>
          </div>
        </div>
      </Toast>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-1">
          ...
          />
          <p className="text-sm text-muted-foreground mt-1">
            Добавьте до 5 изображений, чтобы показать макеты или идеи
          </p>
        </div>
